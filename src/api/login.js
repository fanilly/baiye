...
export default function(userToken,goLogin) {
  return new Promise((resolve, reject) => {
    ...
    if (isDev) {
      ...
      axios.post('https://gbfmall.zzebz.com/index/login/get_user_info',{
        user_token:userToken
      }).then(res => {
        if (res.data.code == 0) {
          location.href = 'https://gbfmall.zzebz.com/index/login/login?jump_url=' + location.pathname.replace(/\//g, '^');
